<template>
  <basic-dialog title="ページを追加" v-model="visible" class="min-w-[400px] outline-none">
    <div>
      <div v-for="(category, index) in categories" :key="index" class="mb-4">
        <div class="flex items-center">
          <puzzle-piece-icon class="w-4 h-4 mr-2"></puzzle-piece-icon>{{ category.category }}
        </div>
        <div class="border-b-[1px] mb-2" v-if="category.category !== ''"></div>
        <div v-for="(type, index2) in category.types" :key="index2" class="flex gap-4" @click="handleSelect(type.type)">
          <div
            class="flex w-full flex-row items-center min-w-32 h-6 text-gray-600 hover:bg-gray-200 cursor-pointer rounded p-4"
          >
            <component :is="type.icon" class="h-6 w-6 mr-1" />
